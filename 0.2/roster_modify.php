...
?>
      ...
      <select name="level">
<?php
/* Reading levels from the given file */
$level_file = fopen("level.txt","r");
while($line = fgets($level_file))
{
	$line = str_replace("\r\n", "", $line);
	/* echoing an option for each line in the file */
	echo "        <option value=\"$line\"";
	
	/* prints "selected" if row chosen from view_roster.php to be modified has a level equal to the value of the line read */
...
<?php
/* Reading programming languages from the given file */
	/* Similar to above */
